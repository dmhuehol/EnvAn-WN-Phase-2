%%ASOSimportFiveMin
    ...
    %direction, present weather conditions, temperature, dewpoint,
    %altimeter setting, relative humidity, visibility, and sky condition
    %ASOSstruct: structure containing all possible ASOS entries--all
    %entries from above, plus station ID, extra ID, record length,
    %day/month/year, HH:MM:SS, observation frequency, another ID, Zulu
    ...
    %using MATLAB, see ASOSdownloadFiveMin.
    %
...
%This function will usually miss 3-5 entries in a month of data.
%If the file does not import properly, or to refine the regular expression,
%use regexr.com

%% Datatype Conversions
usefulStruct = struct([]); %Form a blank structure
% ((VRB)){0,1}(\d{2})(\d{0,3})([GQ]{0})(\d{0,3})(KT)
windExp = '(?<Variable>(VRB){0,1})(?<WindDirection>\d{2})(?<WindSpeed>\d{0,3})(?<WindCharacter>[GQ]{0,1})(?<WindCharacterSpeed>\d{0,3})(KT)';
variableWindExp = '(?<MinimumWindDirection>\d{3})(V)(?<MaximumWindDirection>\d{3})\s?';
errorCount = 0;
errorThreshold = length(ASOSstruct)*0.08; %Unacceptable threshold for errors; don't mess with this unless you have a REALLY GOOD REASON
for count = length(ASOSstruct):-1:1 %Backwards so the entire structure is built in the first step and is then merely filled with data as the loop progresses (saves time)
    try %Prevents a small number of errors from choking the function
        wind(count) = regexp(ASOSstruct(count).Wind,windExp,'names');
    ...
    catch ME; %#ok %If the regular expression fails, then wind data was not recorded for this time, and we manually fill in the missing data the same way that missing data is usually filled in automatically
        wind(count).Variable = 'M'; %M for murder (actually for missing)
        wind(count).WindDirection = ''; %blank
        wind(count).WindSpeed = ''; %blank
        wind(count).WindCharacter = 'M'; %M for mamba
        wind(count).WindCharacterSpeed = ''; %blank
        variableWind(count).MinimumWindDirection = ''; %blank
        variableWind(count).MaximumWindDirection = ''; %blank
        if strcmp(wind(count).Variable,'M')==1 %If the data is missing
            %disp(count) %uncomment to see where errors are occurring
            ...
            if errorCount>errorThreshold %If the error count is greater than the maximum acceptable threshold
                msg = 'Error count exceeded maximum allowable value! Check dataset to ensure compatibility with function.';
                error(msg); %End the function and warn the user
            else
                %do nothing
            end
        end
...
for count = length(ASOSstruct):-1:1
    ...
    usefulStruct(count).Day = sscanf(ASOSstruct(count).Day,'%f');
    usefulStruct(count).Hour = str2num(ASOSstruct(count).Time(1:2)); %#ok %str2double and sscanf both fail here, sometimes str2num is just more robust
    usefulStruct(count).Minute = str2num(ASOSstruct(count).Time(3:4)); %#ok %str2double and sscanf both fail here, sometimes str2num is just more robust
    ...
    try %This section converts negative readings in temperature and dewpoint from M-designated string negatives to actual numbers
        ...
        elseif strcmp(ASOSstruct(count).Temperature(1),'M')==1 && length(ASOSstruct(count).Temperature)>2 %If the temperature is negative
            ...
            ASOSstruct(count).Dewpoint(1) = []; %Delete the M in dewpoint as well (since Td<=T)
